(function(){
	...
	function _filter(object){
		...
		for (var i=0, len = arrKey.length; i<len; i++) {
			...
			sql += key + op;
			if (_isNumber(arrVal[i])) {
				sql += arrVal[i];
			}else if (_isString(arrVal[i])) {
				sql += "'" + arrVal[i] + "'";
			}else if (_isArray(arrVal[i])) {
				sql += "(" + arrVal[i].join(",");
			}
			sql += " AND ";
		}
		...
		if ( _isArray(obj) ) {
			...
			for (var key in obj) {
			  	if ( obj.hasOwnProperty(key) ) {
			  		...
			  		if ( _isObject(_obj) ) {
			  			arguments.callee.apply(this, [_obj, arrKey, arrVal, key]);
			  		}else if ( _isArray(_obj) ) {
			  			// array not handle here
			  			arrKey.push( (_isEmpty(prefix)) ? key : prefix + '_' + key);
			  			arrVal.push(_obj);
