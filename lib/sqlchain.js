(function(){
	...
	sqlchain.pool = _(function(cf){
		...
		this.pl.getConnection(function(err, connection){
			if (err) {throw err;}
			sqlchain.conn = connection; // cache the connection, use array later
		});
	});

	// start transaction here
	sqlchain.start = function(){
		if (this.autocommit){
			...
			if (this.conn) {
				...
				throw err('no connection found!');
			}
		}else {
			throw err('already disable auto-commit!');
		}
	};
	// commit transaction here
	sqlchain.commit = function(){
		if (this.autocommit){
			throw err('already enable auto-commit!');
		}else{
	...
	sqlchain.run = function(sqlOrFunc, func){
		if (_isNull(sqlOrFunc) || _isFunction(sqlOrFunc)) {
			...
			console.log("[sql] ", sqlE);
			__exec(this.conn, sqlE, sqlOrFunc);
